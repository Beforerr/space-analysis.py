...
```{python}
OBS_GROUPS_AND_INSTRS_FNAME =  "obs_groups_and_instrs.yaml"
...
```{python}
#| output: false
def save_obs_groups_and_instrs(fname=OBS_GROUPS_AND_INSTRS_FNAME, **kwargs):
    obs_groups_and_instrs = cdas.get_observatory_groups_and_instruments(**kwargs)

    # save to a file
    with open(fname, "w") as file:
        yaml.dump(obs_groups_and_instrs, file)
```
...
with open(OBS_GROUPS_AND_INSTRS_FNAME, "r") as file:
    obs_groups_and_instrs: list[dict] = yaml.safe_load(file)
